...
function Header({ id }: { id: number }) {
  ...
  const { data, error } = useSWR<HWItem | HWComment | null>((id && inView) ? ['/item', id] : null, async (api, id) => {
    ...
    <div ref={ref} className='user-container'>
      ...
      <div>
        {(data?.type === 'comment' && data.text) && (
          <>
            <div>Comment:</div>
            <div dangerouslySetInnerHTML={{ __html: sanitize(data.text) }}></div>
          </>
        )}
      </div>
    </div>
        ...
        <div>
          ...
          <div>
            ...
            {data.about &&
              <>
                <h3>about:</h3>
                <div className="about" dangerouslySetInnerHTML={{ __html: sanitize(data.about) }}>
                </div>
              </>}
